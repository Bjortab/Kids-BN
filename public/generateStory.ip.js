  ...
  try {
    ...
    const compactHistory = previousChapters
      ...
      .map((txt, idx) => {
        ...
        return `Kapitel ${num}: ${shorten(txt, 280)}`;
      })
      .join("\n\n");
    ...
    const userPromptParts = [
      ...
      chapterRole === "chapter_middle"
        ? [
            ...
            `Starta inte en ny saga, introducera inte en helt ny berättelse.`,
            `Fortsätt där förra kapitlet slutade, med samma huvudpersoner, samma magiska objekt och samma huvudproblem.`,
      ...
      chapterRole === "chapter_final"
        ? [
            ...
            `Lös huvudkonflikten på ett tydligt och barnvänligt sätt.`,
      ...
      `- Använd inte exakt samma inledning ("Det var en solig morgon..." osv) i varje kapitel. Variera miljöer och känslor.`,
      `- Djur ska bete sig rimligt (hästar slickar inte händer som hundar gör, osv).`,
    ...
    const payload = {
      model,
      temperature: 0.7, // lite lugnare för bättre konsistens
