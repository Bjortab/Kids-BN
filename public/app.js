...
(function () {
  ...
  // Normalize age for API - remove "år", replace en-dash/em-dash with hyphen
  ...
  // Find elements with data-id first, fallback to legacy selectors
    ...
    try {
      ...
      try {
        ...
        if (data?.story && data.story.trim()) {
          ...
          throw new Error("Tom saga returnerad");
        }
      } catch (e2) {
        console.error("[BN] createStory fel:", e1, e2);
        const errMsg = "Kunde inte skapa berättelse. Vänligen försök igen.";
  ...
  log("app.js GC v1.6 laddad (age-normalize + robust selectors)");
})();
